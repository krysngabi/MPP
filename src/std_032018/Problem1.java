...
public class Problem1 {
	...
	public static List<Customer> specialCustomers(List<Account> accts) {
		...
		return accts.stream()
				.filter(acc->acc.getCustomer().getCheckingAccount().getId().length()
						==acc.getCustomer().getAddress().getCity().length())
				.map(emp->{
					return new Customer(emp.getCustomer().getCustId(),
							emp.getCustomer().getFirstName(),
							emp.getCustomer().getLastName());} )
	...
	public static List<Customer> specialAccounts(List<Customer> custs) {
		return custs.stream()
				.filter(cust->cust.getCheckingAccount().getBalance()>50 && cust.getCheckingAccount().getBalance()<100 && cust.getFirstName().startsWith("A"))
				.map(emp->{
					return new Customer(emp.getCustId(),
							emp.getFirstName(),
							emp.getLastName());} ).sorted(Comparator.comparing(Customer::getLastName))
				.collect(Collectors.toList());
		//implement          
	}

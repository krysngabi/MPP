...
public class Main {
	...
	public static List<Course> coursesTaughtByValentine(List<Student> allStudents) {
		...
		for(Student st:allStudents) {
			Transcript trans=st.getTranscript();
			List<TranscriptEntry>tEntries=trans.getTentries();
			for(TranscriptEntry  te:tEntries){
				if(te.getCourse().getPrimaryProfessor().equals("Valentine")) {
					coursesFound.add(te.getCourse());
				}
				
			}
		}
		coursesFound=allStudents.stream()
				.flatMap(st->{
					return st.getPlanOfStudy().getCourses().stream()
							.filter(c->c.getPrimaryProfessor().equals("Valentine"));
				}).distinct().collect(Collectors.toList());
		//implement
		
		
	  
		//return coursesFound.stream().distinct().collect(Collectors.toList());
		return coursesFound;

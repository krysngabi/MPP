...
public class Problem1 {
	...
	public static List<String> elementsInJustOne(List<String> list1, List<String> list2) {
		...
		return Stream.concat(list1.stream().filter(e->!list2.contains(e)), 
				list2.stream().filter(e->!list1.contains(e))).distinct().collect(Collectors.toList());
		
		//return Stream.concat(list1.stream(), list2.stream())
		//		.filter( i -> (list1.contains(i) && !list2.contains(i)) || (list2.contains(i) && !list1.contains(i)) )
	...
	public static List<String> getAllFairfieldCustomers(List<Customer> list) {
		// implement
		return list.stream().filter(cus -> cus.getCity()=="Fairfield")
				.map(cus -> cus.getName()).sorted().collect(Collectors.toList());
	}
	...
	private static void testFirstMethod() {
		...
		List<String> list2 = Arrays.asList(new String[] { "A", "V", "C", "W" });
